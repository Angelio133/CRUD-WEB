...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($input_telephone)){
        $telephone_err = "Please enter the salary amount.";     
    } elseif(!ctype_digit($input_telephone)){
        $telephone_err = "Please enter a positive integer value.";
    } else{
    ...
    if(empty($name_err) && empty($username_err) && empty($sexe_err) &&  empty($address_err) && empty($telephone_err) && empty($salary_err) && empty($password_err)){
        ...
        $sql = "UPDATE employees SET name=?, username=?, sexe=?, address=? , telephone=?, salary=?, password=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssssi", $param_name, $param_username, $param_sexe ,$param_address, $param_telephone, $param_salary ,$param_password, $param_id);
